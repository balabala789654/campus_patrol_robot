...
//	GPIO_Init(GPIOA,&GPIO_InitStructure); //初始化PA0
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_1; //GPIOE13
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF;//复用功能
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;	//速度100MHz
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP; //输出
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_DOWN; //上拉
	GPIO_Init(GPIOA,&GPIO_InitStructure); //初始化PE13
  ...
  TIM5_ICInitStructure.TIM_ICFilter = 0x06;//IC1F=0000 配置输入滤波器 不滤波
...
{ 		    
//		if(TIM_GetITStatus(TIM5, TIM_IT_CC1) != RESET)//溢出中断
//	{
//			if((TIM5->CR1 & 0x0010) == 0x0000)
//			{
//					EncodeCound+=1;
//			}
//			if((TIM5->CR1 & 0x0010) == 0x0010)
//			{
//					EncodeCound-=1;
//			}
//	}
//	TIM_ClearITPendingBit(TIM5, TIM_IT_CC1);  //清除TIMx更新中断标志
...
void get_distance()
{
	...
	if(TIM5CH1_CAPTURE_STA&0X80)
	{
		...
		Distance*=0xFFFFFFFF;              //溢出时间总和
		Distance+=TIM5CH1_CAPTURE_VAL;//高电平总时间
		Distance=Distance*170/1000;//时间*声速/2
